...
int Detach_Head_Aac(ADTS_HEADER * adtsheader)
{
	...
	if ((Adts_Headr_Buf[0] == 0xFF)&&((Adts_Headr_Buf[1] & 0xF0) == 0xF0))    
	{
		...
		adtsheader->aac_frame_length = (((((unsigned int) Adts_Headr_Buf[3]) & 0x03) << 11) 
			| (((unsigned int) Adts_Headr_Buf[4] & 0xFF) << 3)| ((unsigned int) Adts_Headr_Buf[5] & 0xE0) >> 5) ;
...
int WriteStruct_Aac_Tag(unsigned char * Buf,unsigned int  Timestamp,unsigned char AACPacketType)
{
	...
	unsigned char SoundRate=3;                
	unsigned char SoundSize = 0x01;         
	...
	if (adts_header.channel_configuration == 0x02)  
	{
		SoundType = 0x01;
	}
	
	audiotag.Type = 0x08;
	audiotag.DataSize = adts_header.aac_frame_length - ADTS_HEADER_LENGTH  + 2; 
	audiotag.Timestamp = Timestamp;
	audiotag.TimestampExtended = 0x00;
	audiotag.StreamID = 0x00;
	audiotag.SoundFormat = 0x0A;   
	...
	Buf[11] = ((audiotag.SoundFormat) << 4) | ((audiotag.SoundRate) << 2)
	| (audiotag.SoundSize << 1) | (audiotag.SoundType);
